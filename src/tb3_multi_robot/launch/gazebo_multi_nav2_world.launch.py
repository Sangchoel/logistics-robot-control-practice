...
def generate_launch_description():
    ...
    robots = [
        {'name': 'tb1', 'x_pose': '-2.2', 'y_pose': '0.4', 'z_pose': 0.01},
        {'name': 'tb2', 'x_pose': '-2.2', 'y_pose': '-2.1', 'z_pose': 0.01},
        {'name': 'tb3', 'x_pose': '0', 'y_pose': '-2.1', 'z_pose': 0.01},
        {'name': 'tb4', 'x_pose': '0', 'y_pose': '0.5', 'z_pose': 0.01},
        # ...
        # ...
        ]
    ...
    world = 'your/path/aws-robomaker-small-warehouse-world/worlds/no_roof_small_warehouse/no_roof_small_warehouse.world'
    ...
    for robot in robots:
        ...
        # Create a initial pose topic publish call with delay and repetition
        # Wait for AMCL to be fully ready, then publish repeatedly
        ...
        # Add delay before initial pose to ensure AMCL is ready
        delay_cmd = ExecuteProcess(
            cmd=['sleep', '3'],
            output='screen'
        )
        
        initial_pose_cmd = ExecuteProcess(
            cmd=['ros2', 'topic', 'pub', '-r', '2', '-t', '8', '--qos-reliability', 'reliable', 
                 '--qos-durability', 'transient_local', namespace[0] + '/initialpose',
        ...
        # Use RegisterEventHandler to ensure robot initialization happens 
        # only after all robots are spawned, with proper delay
        ...
        delayed_init_event = RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=delay_cmd,
        ...
        ld.add_action(delayed_init_event)
        ld.add_action(declare_params_file_cmd)
    ######################
